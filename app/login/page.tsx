...
export default function LoginPage() {
  ...
  // CẬP NHẬT: Chuyển từ email sang username và password
  ...
  // 1. GIỮ NGUYÊN: Tự động Focus vào ô đầu tiên
  useEffect(() => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Gọi API login với cấu trúc dữ liệu mới
      ...
      if (response.ok) {
        ...
        // GIỮ NGUYÊN: Điều hướng mượt mà về trang chủ
        setTimeout(() => {
          window.location.href = "/";
        }, 1500);
      } else {
        // THẤT BẠI: Hiệu ứng rung
        setIsShaking(true);
        setTimeout(() => setIsShaking(false), 500);
        setMessage({ text: `❌ Lỗi: ${data.error || 'Sai tài khoản hoặc mật khẩu!'}`, type: 'error' });
      }
    } catch (error) {
  ...
  return (
    // CẬP NHẬT: Giao diện đồng nhất với trang Register (pt-10, bg-[#F3F3F3])
    <div className="min-h-screen flex flex-col items-center pt-10 font-sans bg-[#F3F3F3] p-4 relative overflow-hidden">
      
      {/* Container Form - Bo góc, bóng đổ và viền đen trên đầu giống trang Register */}
      <div className={`w-full max-w-[420px] bg-white p-8 md:p-12 shadow-2xl border-t-4 border-black animate-in fade-in zoom-in duration-700 ${isShaking ? 'animate-shake' : ''}`}
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-lg font-bold text-gray-800 uppercase tracking-widest">Đăng nhập</h2>
        </div>

        <div className="w-full">
          {/* GIỮ NGUYÊN: CÁC ICON TÍNH NĂNG */}
          ...
          <div className="mt-10 text-center border-t pt-6">
            <p className="text-[10px] text-gray-400 font-bold uppercase mb-6 tracking-widest">Or sign in with</p>
            ...
            <p className="mt-8 text-sm text-gray-500">
              Don't have an account? <a href="/register" className="text-black font-bold hover:underline underline-offset-4">Join for free</a>
            </p>
          </div>
        </div>
      </div>

      {/* GIỮ NGUYÊN: CSS Shake */}
      <style jsx global>{`
        @keyframes shake {
