...
export default function WomenPage() {
  // 1. Trạng thái lưu trữ sản phẩm lấy từ Admin
  const [products, setProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);

  // 2. Hàm gọi dữ liệu từ API Admin
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Thay đổi URL này thành đường dẫn API admin thực tế của bạn
        const response = await fetch("/api/products?category=women&tag=sale");
        const data = await response.json();
        setProducts(data);
      } catch (error) {
        console.error("Lỗi khi lấy sản phẩm từ Admin:", error);
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* --- ĐÃ LOẠI BỎ THANH CHỮ CHẠY "SALE: LAST CHANCE" --- */}

      {/* 2. HERO VIDEO SECTION (GIỮ NGUYÊN - KHÔNG CHỮ ĐÈ) */}
      ...
      {/* 4. GRID SẢN PHẨM: "SALE: SELLING FAST" LẤY TỪ ADMIN */}
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        {loading ? (
          <div className="text-center py-20 font-bold uppercase tracking-widest animate-pulse">Loading items...</div>
        ) : (
