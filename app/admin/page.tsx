...
export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState('users'); // users hoặc products
  ...
  return (
    <div className="min-h-screen bg-[#f3f3f3] font-sans">
      ...
      <div className="max-w-6xl mx-auto p-6 lg:p-10">
        ...
        {/* TAB 1: QUẢN LÝ KHÁCH HÀNG & CỘNG TIỀN */}
        {activeTab === 'users' && (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm font-medium">
                  {/* Ví dụ một dòng khách hàng */}
                  <tr className="border-b">
                    <td className="p-3">customer@example.com</td>
                    <td className="p-3 font-bold text-green-600">$500.00</td>
                    <td className="p-3">
                      <input type="number" className="border p-1 w-24 outline-none focus:border-black" placeholder="+/- $" />
                    </td>
        ...
        {/* TAB 2: QUẢN LÝ SẢN PHẨM (GIÁ, TYM, MÔ TẢ) */}
        {activeTab === 'products' && (
          <div className="bg-white border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] animate-in fade-in duration-300">
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10 p-4 bg-gray-50 border border-dashed border-black">
              <div className="md:col-span-2">
                ...
                <input type="text" className="w-full border p-2 outline-none focus:border-black" />
              </div>
              <div>
                <label className="text-[10px] font-bold uppercase">Giá tiền ($):*</label>
                <input type="number" className="w-full border p-2 outline-none focus:border-black" />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                <label className="text-[10px] font-bold uppercase">Mô tả & Hình ảnh:*</label>
                <textarea className="w-full border p-2 outline-none focus:border-black" rows={2}></textarea>
              </div>
              <button className="md:col-span-2 bg-black text-white py-3 font-black uppercase tracking-tighter hover:bg-gray-800">Đăng sản phẩm mới</button>
            </form>
          </div>
        )}
